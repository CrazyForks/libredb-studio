...
- **Catalog bundle** — `operator/bundle/` (generated by `make -C operator
  bundle`, validated with the operatorframework suite) is the submission
  source for both community catalogs: k8s-operatorhub/community-operators
  (operatorhub.io) and redhat-openshift-ecosystem/community-operators-prod
  (OpenShift console). Channel `operatorhub-community` in
  `distribution/channels.yaml` tracks it (`status: pending` until the first
  listing merges; tracking issue #152).
